...
- אחרי ירוק → `git add .` + `git commit` עם הודעה ברורה בעברית (משמעותית).
- **לפני STOP של כל משימה — חובה לוודא `git status` נקי וקיים commit.** אם יש שינויים לא-ממומשים — commit בטרם עוצרים. (גם בסוף כל ישיבה — repo נקי.)
