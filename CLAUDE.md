...
## וידוא אוטומטי אחרי כל משימה (חובה)
אימות אוטומטי ו-code review הם שלי (Claude) — לא להעביר אותם למשתמש. אחרי כל משימה, במקום רשימת "מה כדאי לבדוק בעצמי":
- להריץ בפועל `dotnet restore` + `dotnet build` + `dotnet test`, לוודא ירוק ולתעד את התוצאה (Count Passed / Failed).
- לבצע code review עצמי מלא לפי כללי הארכיטקטורה, לתקן מה שצריך ולתעד מה תוקן.
- בדיקות אוטומטיות שכתבתי — להריץ אותן ולתעד שנכשלות / עוברות (Red / Green).
- את כל זה לסכם ב-PROGRESS.md (או בדיווח המשימה), לא לבקש מהמשתמש לוודא.

## הרצות Hands-on / אוטומטיות — אני מבצע בעצמי (ברירת מחדל)
- להריץ ולבדוק בעצמי כברירת מחדל (לא להשאיר פקודות בלבד למשתמש):
  - הפעלת השרת (`dotnet run`), שליחת POST חי, בדיקת negotiation — כפי שבוצע באימות 2.1+2.2.
  - כל מה שניתן לבצע בלי יד אנושית — אני חותר אליו בפועל ומתעד את התוצאה.
- **"הרצה ידנית"** = מה שדורש סביבה אנושית בלבד.

## מה נותר למשתמש (רק מה שדורש סביבה אנושית / החלטה)
רשימת "מה צריך לאמת" ב-PROGRESS.md — **רק** מה ש*לא* ניתן לבצע אוטומטית:
- פתיחת דפדפן / צפייה באנדפוינט חי מבחוץ (UI אמיתי).
- אישור שליטה סופי או החלטה אדריכלית.
- (או) דברים שעדיין לא מומשו (TODO).
- עבור כל אלה: להשאיר למשתמש תיעוד/שאלה, לא לבצע במקומו.

## STOP-AND-REPORT
אחרי כל משימה לעצור, להסביר בעברית מה נעשה (כולל תוצאות ה-build/test בפועל ותיקוני ה-code review), ולתת למשתמש לבדוק לפני שעוברים הלאה.
...
## חוק בונוס-כחובה (Cloud-Ready, Docker, K8s, README/ADR, Enhanced UI)
- מה שמוגדר במטלה כ"בונוס" (Cloud-Ready, Docker, Kubernetes, README/ADR, Enhanced UI) — נחשב **חיוני (חובה)** בפרויקט זה. אין דרך "להריץ בלי".
- אסור לדלג על אף אחד מהם בטעם "אין זמן" או "זה אופציונלי".
- כל אחד נכלל במסירה המלאה ויושלם **בסוף, לפני הגשה**.

פרטים מהותיים שמכסה חוק זה (מתוך המטלה):
- Dockerfile multi-stage קטן ל-production + docker-compose (app + redis).
- Kubernetes (deployment.yaml, service.yaml).
- README.md קצר + docs/ADR.md עם הפתרון המוצע לסנכרון בין 5 מופעים (PowerDuplication / Redis pub-sub).
- Enhanced UI בפרונט.
- זה לא חלופה לבחירה; כולם חלק מהגמר — אין ויתור על אף אחת מהן.
